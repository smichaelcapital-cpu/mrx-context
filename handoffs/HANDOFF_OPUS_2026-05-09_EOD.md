...
7. Build the onboarding playbook ledger file at C:\mrx_training_set\_LEDGER\onboarding_playbook.md. Operational step-by-step for onboarding any new CR. Captures gotchas learned tonight: ROUGH binary extraction loses Q/A turn structure (need Stage 1 turn extractor), "FOR MB" folder pattern (CR working directories often mix roughs and finals despite the name), frequency thresholds (HIGH ≥5 depos & ≥20 occurrences / MEDIUM 3-4 & 10-19 / LOW 1-2 or <10), confidence-banded lane structure (AUTO/SUGGEST/FLAG/PROTECTED), protected-class token list, audio-as-evidence-only rule with blank-fill exception. Tomorrow's task — Opus drafts content, Sonnet writes the file.

8. CRITICAL — Build the reporter_pattern_hypotheses.md ledger at C:\mrx_training_set\_LEDGER\reporter_pattern_hypotheses.md. This is the calibration accelerator for CR #2 and beyond. Read tonight's pattern discovery report (C:\mrx_training_set\MB\reports\2026-05-09_split_pattern_discovery.md) and abstract MB's findings into a general hypothesis list. Each entry: pattern name, hypothesis (court reporters likely use this as habitual split trigger), evidence from MB's data, what to look for in next CR's first 1-2 paired depos. The 5 HIGH-confidence patterns Sonnet's overnight sweep surfaced (bare "And" Q-opener: 671/all 7 depos; "So" Q-opener: 373/all 7; double-dash interruption: 244/all 7; trailing "huh?": 101/6 of 7; "Now," Q-opener: 54/6 of 7) are the seed entries. From this point forward, every recon on every reporter updates this file. The flywheel: each CR makes the next faster. By CR #5, calibration probably needs only 3-4 paired depos because most patterns are already known hypotheses. Do not lose this — Scott explicitly flagged at session end.
